 ...
 $pg_head = 'training_list';
	...
	$str = mysqli_query($con, "select * from training_schedules where course_training_id = '".$edit_id."'");
    ...
    <div class="container-fluid">
        ...
        <?php show_message();?>
            ...
            <div class="btn-toolbar">
				...
				<?php if($result['training_stat']=='O'){ ?>
               <button class="btn" type="reset"  onClick="javascript:location.href='<?php echo pg_link("mode=training&pg=open_training_list")?>';">Back</button>
			   <?php }else{ ?>
               <button class="btn" type="reset"  onClick="javascript:location.href='<?php echo pg_link("mode=training&pg=closed_training_list")?>';">Back</button>
			   <?php
			   }
			   ?>
                    <div class="btn-group">
            ...
            <div class="well">
                ...
                <div id="myTabContent" class="tab-content">
                    ...
                    <div class="tab-pane active in" id="home">
                        ...
                        <?php $corse = mysqli_query($con, "select * from course where course_id!= 1 order by course_id desc");
                         ...
                         <?php */?>
                          ...
                          <select name="course_id" id="course_id" >
                           ...
                           <option value="3" <?php if(isset($result) && $result['course_id']==3){?> selected="selected" <?php }?> >Advance Navigation Course</option>
                            <option value="2" <?php if(isset($result) && $result['course_id']==2){?> selected="selected" <?php }?>>PPCDL under MCC</option>
                                                        
                      </select>
                         
                         
                        <label><strong>Start Date</strong></label>
                        <input type="text" value="<?php echo $result['start_date']?>"  name="start_date" id="start_date" class="input-xlarge">
                        
                        <label><strong>End Date</strong></label>
                        <input type="text" value="<?php echo $result['end_date']?>"  name="end_date" id="end_date" class="input-xlarge">
                        
                        <label><strong>Day Type</strong></label>
           				<label class="radio"><input type="radio"  value="Week Days"  name="day_type" id="radio1" class="input-radio" <?php if($result['day_type']=='Week Days'){?> checked="checked" <?php }?> >Week Days</label>
            
            			<label class="radio"><input type="radio"  value="Wk End"  name="day_type" id="radio2" class="input-radio" <?php if($result['day_type']=='Wk End'){?> checked="checked" <?php }?>>Week End</label>
                       
                    
                        <label><strong>Limit Stat</strong></label>
           				<label class="radio"><input type="radio"  value="Y" onclick="groupon();" name="limit_stat" id="radio3" class="input-radio" <?php if($result['limit_stat']=='Y'){?> checked="checked" <?php }?> >Yes</label>
            
            			<label class="radio"><input type="radio"  value="N" onclick="groupon();" name="limit_stat" id="radio4" class="input-radio"  <?php if($result['limit_stat']=='N'){?> checked="checked" <?php }?>>No</label>
                        
                        <div id="l_stat" <?php if($result['limit_stat'] == 'Y'){?>style="display:block;" <?php }else{?>style="display:none;" <?php }?>>
                            <label><strong>Max limit</strong></label>
                            <input type="text" value="<?php echo $result['max_limit']?>"  name="max_limit" id="max_limit" class="input-xlarge">
                        </div>
                        
                        
                        
                        <label><strong>Training Status</strong></label>
                        <label class="radio"><input type="radio"  value="O"  name="training_stat" id="radio5" class="input-radio" <?php if($result['training_stat']=='O'){?> checked="checked" <?php }?>>Open</label>
            
            			<label class="radio"><input type="radio"  value="C"  name="training_stat" id="radio6" class="input-radio" <?php if($result['training_stat']=='C'){?> checked="checked" <?php }?>>Closed</label>
