...
if(isset($edit_id)){
	$str33 = mysqli_query($con,"select * from training_schedules where course_training_id = '".$edit_id."'");
	$result3 = mysqli_fetch_array($str33);
	if($result3['stat']=='N')
		$ch_stat	=	'Y';
	else
		$ch_stat	=	'N';	
        ...
        <div class="row-fluid">
         ...
         <?php show_message();?>
            ...
            <div class="btn-toolbar">
                <a href="<?php echo pg_link("mode=training&pg=add_schedule")?>"  title="Add New Faculty" class="btn btn-primary"><i class="icon-plus"></i> New Schedule</a>
			...
			while($result = mysqli_fetch_array($res_query)){
				...
				?>
                        ...
                        <tr>
                            <td><?php echo $cnt?></td>
                             <td <?php if($result['stat']=='Y'){ ?>style="color:#009900; "<?php } ?>><?php echo $un_qry['course_name']?></td>
                            ...
                            <a href="<?php echo pg_link("mode=training&pg=edit_schedule&edit_id=".$result['course_training_id'])?>" title="Edit"><i class="icon-pencil"></i></a>
							   ...
							   if($result['stat']=='Y'){
							   ?>
                            	<a href="javascript:;;" onClick="javascript:hide_course('<?php echo pg_link("mode=training&pg=training_list&start=".$start."&edit_id=".$result['course_training_id'])?>');" title="hide">Hide this course</a>
                           	<?php
							}
							else
							{
							...
							<a href="javascript:;;" onClick="javascript:hide_course('<?php echo pg_link("mode=training&pg=training_list&start=".$start."&edit_id=".$result['course_training_id'])?>');" title="Show">Show this course</a>
							<?php
							}
							?>
                            </td>
