...
<div class="row-fluid">
    <div class="block span6">
        ...
        <?php $sel_course = mysqli_query($con,"SELECT course_training_id,course_id,COUNT(CASE WHEN training_stat = 'O' THEN 1 END) AS O, COUNT(CASE WHEN training_stat = 'C' THEN 1 END) AS C from training_schedules group by course_id");?>
            <table class="table"  style="width:100%;">
              ...
              <tbody>
               ...
               <?php while($result = mysqli_fetch_array($sel_course)){
			    $qr = mysqli_query($con,"select course_name from course where course_id = '".$result['course_id']."'");
				$res_str = mysqli_fetch_array($qr);?>
                
                <tr>
                  <td ><?php echo $res_str['course_name']?></td>
                  <td><?php echo $result['O']?></td>
                  <td><?php echo $result['C']?></td>
                  
                  <?php 
				  $bok = mysqli_query($con,"select count(*) as c from booking_details where course_id = '".$result['course_id']."' and created_date = '".date('Y-m-d')."'");
				      $cnt_buk = mysqli_fetch_array($bok);?>
                  <td><?php echo $cnt_buk['c']?></td>
                </tr>
              <?php }?>  
