...
if($_POST)
{
	...
	if($del_id){
	$str153 = mysqli_query($con, "delete from refresher_training_schedules where booking_id ='".$del_id."'");
        ...
        <div class="row-fluid">
         ...
         <?php show_message();?>
            ...
            <div class="btn-toolbar">
              <a href="<?php echo pg_link("mode=booking&pg=add_details_for_practical_refresher_course&course_id=3")?>"  title="Add New Booking" class="btn btn-primary"><i class="icon-plus"></i> New Booking Practical Refresher</a> 
			...
			while($result = mysqli_fetch_array($res_query)){
				...
				$univ = mysqli_query($con, "select * from refresher_training where refresher_training_id ='".$result['refresher_training_id']."'");
				$result_bat3 = mysqli_fetch_array($univ);
				?>
                        ...
                        <tr>
							...
							<td><?php echo $result['course_to_attend']?></td>
                            <td><?php echo $result['booking_date']?></td>
                            <td><?php echo date("g A", strtotime($result_bat3['start_time']))?> <?php echo date("g A", strtotime($result_bat3['end_time']))?></td>
							<td><?php if($result['booking_status']=='Y')
										echo 'Paid';
										else
											echo 'Unpaid';
						?></td>
                            ...
                            <td>
                            <a href="<?php echo pg_link("mode=booking&pg=edit_booking_list_of_practical_refresher_course&edit_id=".$result['booking_id'])?>" title="Edit"><i class="icon-pencil"></i></a>
                            </td>
                            <td>
