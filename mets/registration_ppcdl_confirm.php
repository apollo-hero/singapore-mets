...
include("includes/config.php");
	...
	$str15 = mysqli_query($con, "select * from booking_details where booking_id= '".$_GET['booking_id']."'");
			...
			if($bookingDetails['course_name'] == "Cruise"){
				$str15 = mysqli_query($con, "select * from cruise_details where booking_id= '".$_GET['booking_id']."'");
			$cruiseDetails = mysqli_fetch_array($str15);
			}
?>
<!DOCTYPE html>
<html>
...
<form class="form-horizontal" action="payment/paypal.php?ref_booking_id=<?=$_GET['booking_id']?>" method="post">
 ...
 <h5 class="h5_sub_title">PPCDL Course Details</h5>
  ...
  <div class="form-group">
    <label class="control-label col-sm-4">To Attend:</label>
    <div class="col-sm-8">
       <?php echo $bookingDetails['course_name'];?>
    </div>
  </div>
  
  <?php if($bookingDetails['course_name'] == "PPCDL under MCC" || $bookingDetails['course_name'] == "Advance Navigation Course"){ ?>
  <div class="form-group">
  ...
  <?php } ?>
      ...
      <div class="<?php if($bookingDetails['course_name']!="Cruise"){echo "none";} ?>">

  <div class="form-group">
    <label class="control-label col-sm-4">Name:</label>
     <div class="col-sm-8">
       <?php echo $cruiseDetails['name'];?>
    </div>
    ...
    <label class="control-label col-sm-4">Number of pax:</label>
     <div class="col-sm-8">
       <?php echo $cruiseDetails['number_of_pax'];?>
    </div>
    ...
    <label class="control-label col-sm-4">Date:</label>
     <div class="col-sm-8">
       <?php echo $cruiseDetails['cruise_date'];?>
    </div>
	
  </div>
    <div class="form-group">
    <label class="control-label col-sm-4">Timing:</label>
     <div class="col-sm-8">
       <?php echo $cruiseDetails['cruise_timing'];?>
    </div>
...
<div class="voucher <?php if($bookingDetails['registering_as']!="Groupon"){echo "none";} ?>">
 ...
 <div class="col-sm-offset-3 col-sm-9">
  ...
  <div class="col-sm-8">
       <?php echo "$".$bookingDetails['amount'];?>
    </div>
  </div>
  </div>
  <div class="form-group">
    <div class="col-sm-offset-3 col-sm-9">
	<input type="hidden" value="ppcdl" name="type">
	<input type="hidden" value="<?php echo $_GET['booking_id']; ?>" name="booking_id">
	<?php
		if($bookingDetails['registering_as'] != "FAVE"){
	?>
      	<button type="submit" class="btn btn-default">Payment</button>
      <?php }else{
      	?>
      	<a href="register_return.php" class="btn btn-default">Confirm</a>
      	<?php
      }
      ?>
    </div>
