 ...
 $pg_head = 'booking_list_of_PPCDL_under_MCC';
 	if($_POST)
	{
					...
					if($old_course_training_id <> $$course_training_id)
					{
						...
						mysql_query($qr);
						$str152 = mysql_query("select * from training_schedules where course_training_id= '".$old_course_training_id."'");
    ...
    <div class="container-fluid">
        ...
        <?php show_message();?>
            ...
            <div class="btn-toolbar">
                ...
                <button class="btn" type="reset"  onClick="javascript:location.href='<?=pg_link("mode=booking&pg=booking_list_of_PPCDL_under_MCC")?>';">Back</button>
                    <div class="btn-group">
                    </div>
            </div>
                    ...
                    <div class="tab-pane active in" id="home">
                        ...
                        <input type="hidden" name="course_id" id="course_id" value="2" />
                         <label>Preferred Date</label>
                            <input type="text" value="<?=$result['preferred_date']?>"  name="preferred_date" id="preferred_date" class="input-xlarge" readonly="readonly">
                       <div id="right_schedule">
                     
							
                     <?      
  $schedule = mysql_query("select * from training_schedules where course_id='2'  and start_date >= '".date("Y-m-d")."' and training_stat='O' and stat='Y' order by start_date asc");?>
                       ...
                       <input type="hidden" name="course_training_id" id="course_training_id" value="<?=$course_training_id?>" />
                         <input type="hidden" name="old_course_training_id" id="old_course_training_id" value="<?=$result['course_training_id']?>" />
                        </form>
                    </div>
