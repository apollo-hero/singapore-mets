            ...
            <div class="well">
                ...
                <div id="myTabContent" class="tab-content">
                    ...
                    <div class="tab-pane active in" id="home">
                        ...
                        <?php $corse = mysql_query("select * from course order by course_id desc");
						      while($sel_course = mysql_fetch_array($corse)){?>
                              	<option value="<?=$sel_course['course_id']?>" <? if($sel_course['course_id']==$result['course_id']){?> selected="selected" <? }?>><?=$sel_course['course_name']?></option>
                         <? }?>
                         </select>
  ...
  $schedule = mysql_query("select * from training_schedules where course_id = '".$result['course_id']."' order by course_training_id asc ");
					         ...
					         while($show_schedule = mysql_fetch_array($schedule)){
                              ...
                              <div class="tr-schedule">   
                             	<br /><a href="javascript:;;" onclick="add_date('<?=$strt_date[2]."-".$end_date[2]." ".$mnth."(".$show_schedule['day_type'].")"?>');"><?=$strt_date[2]."-".$end_date[2]." ".$mnth."(".$show_schedule['day_type'].")"?></a>
                                </div>
                       <? }?>
                       
           <label>Preferred Date</label>
            <input type="text" value="<?=$training_id?>"  name="training_id" id="training_id" class="input-xlarge">
            
            <label>Are you registering as</label>
            <label class="radio"><input type="radio" onclick="groupon();" value="Full-time Student/NSF"  name="register" id="radio1" class="input-radio" <? if($result['registering_as']=='Full-time Student/NSF'){?> checked="checked" <? }?>>Full-time Student / NSF  $270 </label>
            
            <label class="radio"><input type="radio" onclick="groupon();" value="Group of 3 or more"  name="register" id="radio2" class="input-radio"  <? if($result['registering_as']=='Full-time Student/NSF'){?> checked="checked" <? }?>>Group of 3 or more  $300 </label>
            
            <label class="radio"><input type="radio" onclick="groupon();" value="Working Adult"  name="register" id="radio3" class="input-radio"  <? if($result['registering_as']=='Working Adult'){?> checked="checked" <? }?>>Working Adult  $340 </label>
            
            <label class="radio"><input type="radio"  onclick="groupon();" value="Groupon/Deal.com/Other Vouchers"  name="register" id="radio4" class="input-radio"  <? if($result['registering_as']=='Groupon/Deal.com/Other Vouchers'){?> checked="checked" <? }?>>Groupon / Deal.com / Other Vouchers  $168 / $188(Only at MCC)</label>
   ...
   <? }  ?>
                        ...
                        <input type="text" value="<?=$result['voucher_code']?>"  name="voucher_code" id="voucher_code" class="input-xlarge">
                        
                        <label>Groupon Security Code</label>
                        <input type="text" value="<?=$result['groupon_security_code']?>"  name="security_code" id="security_code" class="input-xlarge">
                        </div>
